...
/* ───────────────────────────────────────────────────────────────
 * Montgomery ladder iç fonksiyonları (projective koordinatlar)
 * ─────────────────────────────────────────────────────────────── */

/* xc_double: Nokta 2*(X1:Z1) → (X3:Z3)
 * Montgomery 1987 formüllerine göre:
 *   X3 = (X1^2 - Z1^2)^2
 *   Z3 = 4*X1*Z1 * (X1^2 + A*X1*Z1 + Z1^2)
 * A = 486662 (Curve25519 katsayısı)
 */
...
/* xc_diffadd: Farksal toplama — (X2:Z2) + (X3:Z3) → (X5:Z5)
 * Başlangıç farkı (X1:Z1) biliniyor.
 */
...
/* projective_ladder: 254-bit Montgomery merdiveni (bit 253 → 0)
 *
 * Başlangıç: (xm:zm) = q (= 1*P), (xm1:zm1) = infinity
 * Bit 254 her zaman 1 (c25519_prepare garantisi) → başlangıç durumu
 * 2*P'yi örtük olarak kodlar.
 *
 * Constant-time: f25519_select ile dallanma yok.
 */
static void projective_ladder(uint8_t *xm,  uint8_t *zm,
